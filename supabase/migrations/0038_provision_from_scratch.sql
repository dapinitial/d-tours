-- Slice 7: provision a trip FROM SCRATCH by default (a single starter chapter), with the
-- Seattle/PNW template as an opt-in. Replaces the 2-arg provision_trip with a 3-arg one
-- (p_from_template default false), so existing 2-arg callers resolve to scratch mode.
drop function if exists provision_trip(text, text);

create or replace function provision_trip(p_intent text, p_name text, p_from_template boolean default false)
...
begin
  ...
  if p_from_template then
    ...
    insert into chapters (id, tenant_id, name, emoji, blurb, sort)
    values ('start_' || suffix, new_tid, 'Day 1', '📍', 'Your trip starts here — drop your first stop in the itinerary.', 1);
  end if;
...
revoke all on function provision_trip(text, text, boolean) from public, anon;
grant execute on function provision_trip(text, text, boolean) to authenticated;
